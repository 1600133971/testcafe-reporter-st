  ...
  return {
    ...
    reportTaskDone: function reportTaskDone(endTime, passed, warnings) {
      ...
      this
        ...
        .write(this.chalk.green('[ DURATION ] ') + (day > 0 ? day + ' days, ' : '') + (hour > 0 ? hour + ' hours, ' : '') + (minute > 0 ? minute + ' minutes, ': '') + (second > 0 ? second + ' seconds': '')).newline()
        .write(this.chalk.green('[  PASSED  ] ' + passed + ' tests')).newline()
        .write(this.chalk.red('[  FAILED  ] ' + this.failed + ' tests')).newline()
        .write(this.chalk.grey('[  SKIPPED ] ' + this.skipped + ' tests')).newline()
        .write(this.chalk.yellow('[  WARNING ] ')).newline()
        .setIndent(0);
    ...
    renderErrors: function renderErrors() {
      ...
      this.errDescriptors.forEach(function (errDescriptor) {
        _this3
          .useWordWrap(true).newline()
          .write(_this3.formatError(errDescriptor.err)).newline().newline();
      });
    },
