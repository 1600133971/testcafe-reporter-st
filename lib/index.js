...
exports['default'] = function () {
  return {
    ...
    reportTaskDone: function reportTaskDone(endTime, passed, warnings) {
      ...
      this
        .write(this.chalk.green('[==========] ') + this.stasticTest(this.testCount) + ' from ' + this.stasticFixture(this.fixtureCount) + ' ran.').newline()
    ...
    stasticTest: function stasticNum(num) {
      return num <= 1 ? num + ' test' : num + ' tests';
    },
    stasticFixture: function stasticFixture(num) {
      return num <= 1 ? num + ' test fixture' : num + ' test fixtures';
    },
    ...
    renderWarnings: function renderWarnings(warnings) {
      ...
      warnings.forEach(function (msg) {
        _this.setIndent(1).write(_this.chalk.bold.yellow('--')).newline().setIndent(2).write(msg).newline();
      });
    }
  }
};
